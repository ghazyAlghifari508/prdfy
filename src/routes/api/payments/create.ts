...
import { findTopUpPackage } from "@/lib/constants";
	...
	server: {
		handlers: {
			POST: async ({ request }: { request: Request }) => {
				...
				const topUpPkg = findTopUpPackage(planId);
				const isTopUp = Boolean(topUpPkg);
				...
				const paymentRow = {
					...
					plan: isTopUp && topUpPkg ? topUpPkg.id : planId,
					amount,
					status: "pending",
				};
				if (isTopUp && topUpPkg) {
					...
					try {
						await db.transaction(async (tx) => {
							...
							if (!canPurchaseTopUp(eff)) throw new Error("TOPUP_NOT_ELIGIBLE");
							...
							if (locked.currentPeriodStart && locked.currentPeriodEnd) {
								...
								for (const u of usageRows) {
									const p = findTopUpPackage(u.plan);
									if (p) usedThisPeriod += p.credits;
								}
								if (
									remainingTopUpQuota({
				...
				try {
					const response = await fetch(`${gateway.snapBaseUrl}/transactions`, {
						method: "POST",
						headers: {
