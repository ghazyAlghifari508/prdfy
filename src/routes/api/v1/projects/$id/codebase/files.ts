...
export const Route = createFileRoute("/api/v1/projects/$id/codebase/files")({
	server: {
		handlers: {
			...
			// runs per file at completion. Bearer sync credential only (see
			...
			POST: async ({
				...
				const receivedChunks = replaced ? pathRows.length : pathRows.length + 1;
				const response = {
					status: "uploading",
					snapshotId: snapshot.id,
					path: body.path,
